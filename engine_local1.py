...
def train_one_epoch1(model: torch.nn.Module,model_teacher: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for _ in metric_logger.log_every(range(len(data_loader)), print_freq, header):
        with torch.no_grad():
            outputs_teacher1 = model_teacher(samples, teacher=True)

        outputs = model(samples, outputs_teacher1, teacher=True)
        loss_dict = criterion(outputs, outputs_teacher1, targets)
